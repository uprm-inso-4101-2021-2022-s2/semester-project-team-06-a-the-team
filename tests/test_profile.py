...
class TestProfile(unittest.TestCase):
    def setUp(self):
        ...
        self.victor_Tutor1 = Tutor("Yosemite Tee",15.00,"L","Sportswear","Men","Basic T-shirt for everyday use","../images/yosemite_tee.png", self.victor)
    ...
    def test_init_values(self):
        # TODO: Tutors - could add test to it, if we add a limit to the amount of Tutors in the list
    ...
    def test_add_Tutor_to_sell(self):
        #Tutor should be of type Tutor
        test_Tutors = [1.0, 1, True, 'hello', [], (), {}] 
        for Tutor in test_Tutors: 
             with self.subTest():
                 with self.assertRaises(TypeError):
                    self.victor_profile._Add_Tutor_to_Sell(Tutor)

        # TODO: the test below fails
        # should not add Tutor if it is owned by another user
        with self.assertRaises(ValueError):
            self.kevin_profile._Add_Tutor_to_Sell(self.victor_Tutor1)
        pass
    ...
    def test_remove_Tutor(self):
        ...
        for Tutor in test_Tutors: 
             with self.subTest():
                with self.assertRaises(TypeError):
                    self.victor_profile._Remove_Tutor(Tutor)

        # TODO: the test below fails
        # should not remove an Tutor if it is owned by another user
        with self.assertRaises(ValueError):
            self.kevin_profile._Remove_Tutor(self.victor_Tutor1)

        # TODO: the test below fails
        # should not remove an Tutor if it is not part of Tutors 
        with self.assertRaises(ValueError):
            self.kevin_profile._Remove_Tutor(self.victor_Tutor1)
